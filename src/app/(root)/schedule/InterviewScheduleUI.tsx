...
const InterviewScheduleUI = () => {
  ...
  const interviewers = useQuery(api.interviews.getAllInterviews) ?? [];
  ...
  const scheduleMeeting = async () => {
    ...
    if (!formData.candidateId || formData.interviewerId.length === 0) {
      toast.error("Please select both candidate an at least one interviewer");
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to create schedule meeting");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const selectedInterviewers = interviewer.filter((i) =>
    formData.interviewerId.includes(i.clerkId)
  );

  const availableInterviewers = interviewer.filter(
    (i) => !formData.interviewerId.includes(i.clerkId)
  );
      ...
      <div className="flex items-center justify-between">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Schedule and menage interviews
          </p>
        </div>
